        ...
        <div key={item._id} className="bg-white p-4 rounded-3xl shadow-sm border border-slate-100 flex justify-between items-center gap-3">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              {item.status === 'booked' && item.returnTime && (
                <div className="mt-1">
                  {/* Replace the static span with the CountdownTimer */}
                  <CountdownTimer returnTime={item.returnTime} />
                  {/* <p className="text-[8px] text-slate-400 mt-0.5 font-bold uppercase tracking-tighter">
                    Until Available
                  </p> */}
                </div>
              )}
