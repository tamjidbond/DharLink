      ...
      <div className="grid grid-cols-1 gap-4 md:gap-6">
              ...
              incomingRequests.map(req => (
                <div key={req._id} className="bg-white p-5 md:p-6 rounded-3xl shadow-sm border border-slate-100 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 hover:border-indigo-100 transition">
                  <div className="w-full">
                    ...
                    <p className="text-indigo-600 text-sm font-bold">{req.borrowerPhone || "No Phone"}</p>
                    <p className="text-slate-500 italic text-sm mt-1 line-clamp-2">"{req.message}"</p>
                  </div>
        ...
        {activeTab === 'borrowing' && (
          <div className="space-y-4">
            ...
            {myBorrowRequests.length === 0 ? <p className="text-slate-400 italic bg-white p-10 rounded-3xl text-center border-2 border-dashed text-sm">No requests found.</p> :
              myBorrowRequests.map(req => (
                <div key={req._id} className="bg-white p-5 rounded-3xl shadow-sm border border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                  <div className="min-w-0">
                    ...
                    <p className="text-[10px] text-slate-400 mt-1 uppercase tracking-widest truncate">Lender: {req.lenderEmail}</p>
                  </div>
